...
- Surprise encountered: test harnesses and some local setups rely on non-standard `XDG_CONFIG_DIR` for config discovery precedence. If you switch to `XDG_CONFIG_HOME`-first resolution without updating tests/env guards, integration tests may silently read a real user config instead of the test fixture.
