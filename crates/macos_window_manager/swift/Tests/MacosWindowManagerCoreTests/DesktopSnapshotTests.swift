...
final class DesktopSnapshotTests: XCTestCase {
    ...
    func testDesktopSnapshotFailsWhenFocusedWindowProbeFails() {
        ...
        XCTAssertThrowsError(try backend.desktopSnapshot())
    }

    func testTopologySnapshotReturnsTopologyWhenFocusedWindowProbeFails() throws {
        let backend = Backend(system: FocusedWindowFailingSystem(
            requiredSymbols: PrivateSymbols.requiredSymbols,
            accessibilityTrusted: true,
            mainConnectionIDValue: 7,
