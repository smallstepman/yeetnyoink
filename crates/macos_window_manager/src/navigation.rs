...
use crate::api::{
    ActiveSpaceFocusTargetHint, MacosNativeApi, NativeBounds, NativeDesktopSnapshot,
    NativeDirection, NativeSpaceSnapshot, NativeWindowSnapshot,
};
use crate::desktop_topology_snapshot::SpaceKind;
use crate::error::{MacosNativeOperationError, MacosNativeProbeError};
...
pub(crate) fn active_space_ax_backed_same_pid_target<A: MacosNativeApi + ?Sized>(
    api: &A,
    snapshot: &NativeDesktopSnapshot,
...
pub(crate) fn focus_same_space_target_in_snapshot<A: MacosNativeApi + ?Sized>(
    api: &A,
    snapshot: &NativeDesktopSnapshot,
